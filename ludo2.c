...
REGRAS GERAIS
- O primeiro jogador é sorteado, apartir disso segue o sentido horário;
...
void dice_animation() {
  turnView();
  difY[0] = (4.5 - 13.5) / 50.0;
  difY[1] = (4.5 - 13.5) / 100.0;
...
void mouse(int button, int state, int x, int y) {
  ...
  case GLUT_LEFT_BUTTON:
    ...
    if (state == GLUT_DOWN) {
      ...
      // joga o dado, tirando um numero aleatorio de 1 a 6
      if (diceCount < MAX_DICE_ROLLS) {
        for (int i = 0; i < diceResult; i++) {
          movePawn(turn, pawnToMove);
          // check if pawn can move
          // check kill
          // check win
        }
      } else {
        // envia a ultima peça mexida de volta para o inicio
      }

      // Se o ultimo dado rolado não for 6, o turno é do próximo jogador
      if (diceResult != 6) {
        turn = (turn + 1) % MAX_PAWNS;
        diceCount = 0;
        turnView();
